...
void gms_matcher::AssignMatchPairs(int GridType) {
	...
	for (size_t i = 0; i < mNumberMatches; i++)
	{
		...
		if (lgidx >= mGridNumberLeft || rgidx >= mGridNumberRight)
			continue;
...
int gms_matcher::run(int RotationType) {
	...
	mvbInlierMask = vector<bool>(mNumberMatches,false);
	...
	mvMatchPairs = vector<pair<int, int>>(mNumberMatches,pair<int, int>(0, 0));
	...
	for (int GridType = 1; GridType <= 4; GridType++) 
	{
		...
		mCellPairs=vector<int>(mGridNumberLeft, -1);
		mNumberPointsInPerCellLeft =vector<int>(mGridNumberLeft, 0);
