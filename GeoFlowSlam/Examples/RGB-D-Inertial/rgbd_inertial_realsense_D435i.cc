...
int main(int argc, char** argv) {
  ...
  //             sensor.set_option(RS2_OPTION_EMITTER_ENABLED, 1); // emitter on
  ...
  // RGB stream
  // 从文件启用设备
  // cfg.enable_device_from_file(string(argv[3]));
  ...
  auto imu_callback = [&](const rs2::frame& frame) {
    ...
    if (rs2::frameset fs = frame.as<rs2::frameset>()) {
      ...
      // cout<<"new_timestamp_image: "<<new_timestamp_image<<endl;
  ...
  while (!SLAM.isShutDown()) {
    ...
    for (int i = 0; i < vGyro.size(); ++i) {
      ORB_SLAM3::IMU::Point lastPoint(vAccel[i].x, vAccel[i].y, vAccel[i].z,
                                      ...
                                      vGyro_times[i]);
      // 输出IMU数据
      // std::cout << "timestamp: " << vGyro_times[i] << std::endl;
      // std::cout << "Accel: " << vAccel[i].x << ", " << vAccel[i].y << ", " <<
      // vAccel[i].z << std::endl; std::cout << "Gyro: " << vGyro[i].x << ", "
      // << vGyro[i].y << ", " << vGyro[i].z << std::endl;
    ...
    if (vImuMeas.size() != 0) {
      ...
      // std::cout << "vImuMeas[0].timestamp: " << vImuMeas[0].t << std::endl;
    }
