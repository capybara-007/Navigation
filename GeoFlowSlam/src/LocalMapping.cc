...
LocalMapping::LocalMapping(System* pSys, Atlas* pAtlas, Settings* settings,
...
void LocalMapping::Run() {
  ...
  while (!bStop) {
    ...
    if (CheckNewKeyFrames() && !mbBadImu) {
      ...
      // For Mono And Mono+IMU
      // if (mpTracker->GetUseOpticalFlow() &&
      ...
      int num_MPs_BA = 0;
      int num_edges_BA = 0;
      bool mbUseICPLocalBA = mpSettings->enableICPLocalBA();
      // cout << "mbUseICPLocalBA: " << mbUseICPLocalBA << endl;
      // bool bUseLidar =
      //     (mpTracker->GetMatchesInliers() < 75) && mpSettings->useLidarObs();
      bool bUseLidar =
          mpSettings->useLidarObs() && mpSettings->enableLidarLocalBA();
      if (!CheckNewKeyFrames() && !stopRequested()) {
        if (mpAtlas->KeyFramesInMap() > 2) {
          if (mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized()) {
            ...
            if (bUseLidar) {
              pcl::PointCloud<PointType>::Ptr localLidarMap =
                  pcl::PointCloud<PointType>::Ptr(
                      new pcl::PointCloud<PointType>());
              mpLidarMapping->GetLocalMap(localLidarMap);
              Optimizer::LocalVisualLidarInertialBA(
                  mpCurrentKeyFrame, localLidarMap, &mbAbortBA, mbUseICPLocalBA,
                  mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA,
                  num_MPs_BA, num_edges_BA, bLarge,
                  !mpCurrentKeyFrame->GetMap()->GetIniertialBA2());
            } else {
              Optimizer::LocalInertialBA(
                  mpCurrentKeyFrame, &mbAbortBA, mbUseICPLocalBA,
                  mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA,
                  num_MPs_BA, num_edges_BA, bLarge,
                  !mpCurrentKeyFrame->GetMap()->GetIniertialBA2());
            }
            ...
            if (bUseLidar) {
              pcl::PointCloud<PointType>::Ptr localLidarMap =
                  pcl::PointCloud<PointType>::Ptr(
                      new pcl::PointCloud<PointType>());
              mpLidarMapping->GetLocalMap(localLidarMap);
              Optimizer::LocalVisualLidarBA(
                  mpCurrentKeyFrame, localLidarMap, &mbAbortBA, mbUseICPLocalBA,
                  mpCurrentKeyFrame->GetMap(), num_FixedKF_BA, num_OptKF_BA,
                  num_MPs_BA, num_edges_BA);
            } else {
        ...
        // Initialize IMU here
        int bFastInit = mpSettings->get_fast_init();
        if (mpSettings->getImuInitMethod() == System::IMU_ORB_SLAM3) {
          if (!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial &&
              !bFastInit) {
            if (mbMonocular)
              InitializeIMU(1e2, 1e10, true, bFastInit);
            else
              InitializeIMU(1e2, 1e5, true, bFastInit);
          }
          if (!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial &&
              bFastInit) {
            InitializeIMU(0.f, 0.f, true, bFastInit);
          }
        } else if (mpSettings->getImuInitMethod() == System::VIG_INIT) {
          if (!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial &&
              !mpCurrentKeyFrame->GetMap()->GetIniertialBA2()) {
            float priorG, priorA;
            priorG = 0.f;
        ...
        } else if (mpSettings->getImuInitMethod() ==
                   System::IMU_INITIALIZATION) {
          if (!mpCurrentKeyFrame->GetMap()->isImuInitialized() && mbInertial &&
              !mpCurrentKeyFrame->GetMap()->GetIniertialBA2()) {
            float priorG, priorA;
            priorG = 0.f;
                                                    ...
                                                    // local-mapping is called
            {
              if (!mpCurrentKeyFrame->GetMap()->GetIniertialBA1()) {
                if (mTinit > 5.0f) {
                  ...
                  if (mbMonocular)
                    InitializeIMU(1.f, 1e5, true, bFastInit);
                  else
                    InitializeIMU(1.f, 1e5, true, bFastInit);
      ...
      if (mpTracker->mSensor == System::RGBD ||
        ...
        for (auto pKF : mlNewKeyFrameForDenseMap)
          mpLidarMapping->insertKeyFrame(pKF);
      }
...
void LocalMapping::CreateNewMapPointsNew() {
  ...
  // Iterate through all feature points in the current frame,
  // get the keyframe where each feature was first observed,
  // then triangulate using the current frame and that keyframe.
  for (int i = 0; i < mpCurrentKeyFrame->track_feature_pts_.size(); i++) {
    if (mpCurrentKeyFrame->track_feature_pts_.at(i)->mp == nullptr) {
      ...
      if (kf == mpCurrentKeyFrame || kf->mnId == 2) continue;

      Sophus::SE3<float> sophTcw2 = kf->GetPose();
      Eigen::Matrix<float, 3, 4> eigTcw2 = sophTcw2.matrix3x4();
      Eigen::Matrix<float, 3, 3> Rcw2 = eigTcw2.block<3, 3>(0, 0);
      Eigen::Matrix<float, 3, 3> Rwc2 = Rcw2.transpose();
      Eigen::Vector3f tcw2 = sophTcw2.translation();
      Eigen::Vector3f Ow2 = kf->GetCameraCenter();

      const float& fx2 = kf->fx;
      const float& fy2 = kf->fy;
      const float& cx2 = kf->cx;
      const float& cy2 = kf->cy;
      const float& invfx2 = kf->invfx;
      const float& invfy2 = kf->invfy;

      GeometricCamera *pCamera1 = mpCurrentKeyFrame->mpCamera,
                      *pCamera2 = kf->mpCamera;
      ...
      const cv::KeyPoint& kp2 = (kf->NLeft == -1) ? kf->mvKeysUn[idx2]
                                : (idx2 < kf->NLeft)
                                    ? kf->mvKeys[idx2]
                                    : kf->mvKeysRight[idx2 - kf->NLeft];

      const float kp2_ur = kf->mvuRight[idx2];
      bool bStereo2 = (!kf->mpCamera2 && kp2_ur >= 0);
      const bool bRight2 = (kf->NLeft == -1 || idx2 < kf->NLeft) ? false : true;

      if (mpCurrentKeyFrame->mpCamera2 && kf->mpCamera2) {
        if (bRight1 && bRight2) {
          sophTcw1 = mpCurrentKeyFrame->GetRightPose();
          Ow1 = mpCurrentKeyFrame->GetRightCameraCenter();

          sophTcw2 = kf->GetRightPose();
          Ow2 = kf->GetRightCameraCenter();

          pCamera1 = mpCurrentKeyFrame->mpCamera2;
          pCamera2 = kf->mpCamera2;
        } else if (bRight1 && !bRight2) {
          sophTcw1 = mpCurrentKeyFrame->GetRightPose();
          Ow1 = mpCurrentKeyFrame->GetRightCameraCenter();

          sophTcw2 = kf->GetPose();
          Ow2 = kf->GetCameraCenter();
      ...
      if (bStereo1)
        ...
        cosParallaxStereo2 = cos(2 * atan2(kf->mb / 2, kf->mvDepth[idx2]));
      ...
      if (cosParallaxRays < cosParallaxStereo && cosParallaxRays > 0 &&
        ...
        countStereoAttempt++;
        bPointStereo = true;
        goodProj = kf->UnprojectStereo(idx2, x3D);
      } else {
...
cv::Mat LocalMapping::ComputeF12(KeyFrame*& pKF1, KeyFrame*& pKF2) {
  Sophus::SE3<float> sophTcw1 = pKF1->GetPose();
  Eigen::Matrix<float, 3, 4> eigTcw1 = sophTcw1.matrix3x4();
  Eigen::Matrix<float, 3, 3> Rcw1 = eigTcw1.block<3, 3>(0, 0);
  Eigen::Vector3f tcw1 = sophTcw1.translation();
  ...
  Eigen::Matrix<float, 3, 3> R12 = Rcw1 * Rcw2.transpose();
  Eigen::Vector3f t12 = -Rcw1 * Rcw2.transpose() * tcw2 + tcw1;

  // cv::Mat t12x = SkewSymmetricMatrix(t12);
  Eigen::Matrix<float, 3, 3> t12x;
  t12x << 0, -t12[2], t12[1], t12[2], 0, -t12[0], -t12[1], t12[0], 0;
  const Eigen::Matrix3f& K1 = pKF1->mK_;
  const Eigen::Matrix3f& K2 = pKF2->mK_;
  Eigen::Matrix<float, 3, 3> F12 =
      K1.transpose().inverse() * t12x * R12 * K2.inverse();
  cv::Mat mat;
  cv::eigen2cv(F12, mat);
  return mat;
}
void LocalMapping::CreateNewMapPoints() {
  ...
  for (size_t i = 0; i < vpNeighKFs.size(); i++) {
    ...
    matcher.SearchForTriangulation(mpCurrentKeyFrame, pKF2, vMatchedIndices,
                                   false, bCoarse);
    // bool bCoarse = mbInertial && mpTracker->mState ==
    // Tracking::RECENTLY_LOST; if(!mpSettings->useOpticalFlow()){
    //   matcher.SearchForTriangulation(mpCurrentKeyFrame, pKF2,
    //   vMatchedIndices,
    //                                  false, bCoarse);
    // }else{
    //   cv::Mat F12 = ComputeF12(mpCurrentKeyFrame, pKF2);
    //   matcher.SearchForTriangulationWithGMS(mpCurrentKeyFrame, pKF2, F12,
    ...
    for (int ikp = 0; ikp < nmatches; ikp++) {
      ...
      // add lyc
      if (mpTracker->GetUseOpticalFlow() &&
          !mpCurrentKeyFrame->track_feature_pts_.empty()) {
...
void LocalMapping::InitializeIMU(float priorG, float priorA, bool bFIBA,
  ...
  cout << "mbg: " << mbg.transpose() << endl;
  cout << "mba: " << mba.transpose() << endl;
  cout << "mRwg: " << mRwg << endl;
...
void LocalMapping::VigInit(float priorG, float priorA, bool bFIBA) {
  ...
  vector<KeyFrame*> vpKF(
      std::prev(lpKF.end(), std::min(min_kf + 10, (int)lpKF.size())),
      lpKF.end());

  // todo, after GlobalBundleAdjustemnt, need to update kf's pose and landmark
  // position
  ...
  if (1) {
    ...
    Eigen::Vector3d bg;
    bool flag =
        Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), bg, 0.0);
    if (flag && bg.norm() > 1.0) flag = false;
  ...
  if (gyroscope_result.success) {
    ...
    if (imuInit.init_imu()) {
      std::cout << std::fixed << std::setprecision(5)
                << "vig_init, num kf=" << vpKF.size()
                << ", bg=" << imuInit.bg.transpose()
      ...
      if ((imuInit.scale > 10. && mbMonocular) ||
          (imuInit.scale > 1.5 && !mbMonocular) || imuInit.scale < 0.1 ||
          ba_max > 3. || ba_min < -3. || imuInit.ba.norm() > 3.0) {
        bInitializing = false;
        return;
      }
      ...
      for (auto it = lpKF.begin(); it != lpKF.end(); it++) {
        (*it)->SetNewBias(b_);
        (*it)->bImu = true;
        if ((*it)->mpImuPreintegrated) (*it)->mpImuPreintegrated->Reintegrate();
      }

      // update velocity
      for (auto it = lpKF.begin(); it != lpKF.end(); it++) {
        if (!(*it)->mpImuPreintegrated) continue;
        if (!(*it)->mPrevKF) continue;

        Eigen::Vector3f _vel =
            ((*it)->GetImuPosition() - (*it)->mPrevKF->GetImuPosition()) /
            (*it)->mpImuPreintegrated->dT;
        (*it)->SetVelocity(_vel);
      ...
      if (1) {
        ...
        if (1) {
          ...
          Eigen::Matrix3f R = Eigen::Quaternionf::FromTwoVectors(gravity, gI)
                                  .toRotationMatrix();  // gI = R * gravity
        ...
        Optimizer::InertialOptimization(mpAtlas->GetCurrentMap(), mRwg, mScale,
                                        mbg, mba, mbMonocular, infoInertial,
                                        false, false, priorG, priorA);

        std::cout << "num kf="
                  << mpAtlas->GetCurrentMap()->GetAllKeyFrames().size()
                  << ", scale=" << mScale << ", bg=" << mbg.transpose()
                  << ", ba=" << mba.transpose() << std::endl;
        if (mScale < 1e-1) {
          cout << "scale too small" << endl;
          bInitializing = false;
          return;
...
void LocalMapping::IMUAlign(float priorG, float priorA, bool bFIBA) {
  ...
  if (gyroscope_result.success) {
    ...
    if (1) {
      // prior matters
      if (1)
        analytic_accelerometer(vpKF, accelerometer_result,
                               gyroscope_result.bias_g.cast<float>(),
                               Eigen::Vector3f::Zero(), 0.0);
      else
        analytic_accelerometer(vpKF, accelerometer_result,
                               gyroscope_result.bias_g.cast<float>(),
                               Eigen::Vector3f::Zero(), 1e5);
    } else {
      // bug
      mqh_accelerometer(vpKF, accelerometer_result,
                        gyroscope_result.bias_g.cast<float>());
    }
    if (accelerometer_result.success) {
      // std::cout << std::fixed << std::setprecision(5)
      //           << "imu_initialization, num kf=" << vpKF.size()
      //           << ", bg=" << gyroscope_result.bias_g.transpose()
      //           << ", ba=" << accelerometer_result.bias_a.transpose()
      //           << ", scale=" << accelerometer_result.scale
      //           << ", gravity=" << accelerometer_result.gravity.transpose()
      //           << std::endl;

      float ba_max = accelerometer_result.bias_a.cast<float>().maxCoeff();
      float ba_min = accelerometer_result.bias_a.cast<float>().minCoeff();

      if ((accelerometer_result.scale > 10. && mbMonocular) ||
          (accelerometer_result.scale > 1.5 && !mbMonocular) ||
          accelerometer_result.scale < 0.1 || ba_max > 3. || ba_min < -3. ||
          accelerometer_result.bias_a.norm() > 3.0) {
        bInitializing = false;
        return;
      }
