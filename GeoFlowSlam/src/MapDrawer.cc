...
void MapDrawer::SetCurrentCameraPose(const Sophus::SE3f &Tcw,
                                     const Sophus::SE3f &ICP_Twc) {
  unique_lock<mutex> lock(mMutexCamera);
  mCameraPose = Tcw.inverse();
  mCameraICPPose=ICP_Twc;
}
...
void MapDrawer::GetCurrentICPOpenGLCameraMatrix(pangolin::OpenGlMatrix &M,
                                             pangolin::OpenGlMatrix &MOw) {
  Eigen::Matrix4f Twc;
  {
    unique_lock<mutex> lock(mMutexCamera);
    Twc=mCameraICPPose.matrix();
  }
...
void MapDrawer::DrawCurrentCameraICP(pangolin::OpenGlMatrix &Twc) {
  const float &w = mCameraSize;
  const float h = w * 0.75;
  const float z = w * 0.6;
