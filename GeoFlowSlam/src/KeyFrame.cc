...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
  ...
  imDepth = F.imageDepth.clone();
  imRGB = F.imageRGB.clone();

  
  mnMatchesInliers = F.mnMatchesInliers;
  source_points = F.source_points;
  mpPointCloudDownsampled = F.mpPointCloudDownsampled;
...
// bool KeyFrame::PredictStateICP(KeyFrame *mCurrentFrame, KeyFrame *mLastFrame)
// {
//   // 使用mpRegistration进行深度点云配准

//   if (mCurrentFrame.imageDepth.empty() || mLastFrame.imageDepth.empty())
//     return false;
//   // 1. 将深度图转换为点云
//   std::vector<Eigen::Vector4f> source_points =
//       mpRegistration->ConvertDepthToPointCloud(mCurrentFrame.imageDepth,
//                                                mCurrentFrame.mK_);
//   std::vector<Eigen::Vector4f> target_points =
//       mpRegistration->ConvertDepthToPointCloud(mLastFrame.imageDepth,
//                                                mLastFrame.mK_);
//   if (source_points.size() < 50 || target_points.size() < 50) {
//     cout << "source_points.size()" << source_points.size() << endl;
//     return false;
//   }
//   // 2. 使用GICP进行配准
//   // cout << "start GICP" << endl;
//   std::chrono::steady_clock::time_point t1 =
//   std::chrono::steady_clock::now(); RegistrationResult result =
//       mpRegistration->RegisterPointClouds(target_points, source_points);
...
//           .count();
//   Eigen::Matrix4d delta_pose = result.T_target_source.matrix();
//   // cout << " Delta pose: " << delta_pose << endl;
//   // cout << "converged:" << result.converged << endl;
//   // cout << "num_inliers:" << result.num_inliers << endl;
//   // cout << "error:" << result.error << endl;
//   if (result.converged && result.num_inliers > 10 && result.error < 50) {
