...
System::System(const string &strVocFile, const string &strSettingsFile,
               ...
               const string &strSequence, const string &save_dir)
    : mSensor(sensor),
  ...
  // std::cout << "here is ok0" <<std::endl;
  if (!node.empty() && node.isString() && node.string() == "1.0") {
    settings_ = new Settings(strSettingsFile, mSensor);
    // std::cout << "here is ok1" <<std::endl;
    if (settings_->atlasLoadFile() != "") {
      std::cout << settings_->atlasLoadFile() << std::endl;
      mStrLoadAtlasFromFile = save_dir + "/" + settings_->atlasLoadFile();
    }
    // mStrLoadAtlasFromFile = settings_->atlasLoadFile();
    mStrSaveAtlasToFile = save_dir + "/" + settings_->atlasSaveFile();

    cout << (*settings_) << endl;
    // std::cout << "here is okk" <<std::endl;
  } else {
    settings_ = nullptr;
    cv::FileNode node = fsSettings["System.LoadAtlasFromFile"];
    if (!node.empty() && node.isString()) {
      mStrLoadAtlasFromFile = save_dir + "/" + (string)node;
      cout << "Load Atlas from: " << mStrLoadAtlasFromFile << endl;
    }

    node = fsSettings["System.SaveAtlasToFile"];
    if (!node.empty() && node.isString()) {
      mStrSaveAtlasToFile = save_dir + "/" + (string)node;
      cout << "Save Atlas to: " << mStrSaveAtlasToFile << endl;
    }
  }

  // std::cout << "here is ok2" <<std::endl;
  ...
  //  mSensor!=MONOCULAR && mSensor!=IMU_MONOCULAR
  mpLoopCloser =
      new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, settings_,
  ...
  // usleep(10*1000*1000);
  int enableMapping = fsSettings["UsePointCloudObs"];
  if ((mSensor == IMU_RGBD || mSensor == RGBD)) {
    cout << "Create LidarMapping" << endl;
    // for point cloud resolution
    float global_resolution = fsSettings["LidarMapping.GlobalResolution"];
    float local_resolution = fsSettings["LidarMapping.LocalResolution"];
    float meank = fsSettings["LidarMapping.meank"];
    float thresh = fsSettings["LidarMapping.thresh"];

    mpLidarMapping = new LidarMapping(global_resolution, local_resolution,
                                      meank, thresh, save_dir);
    mpLocalMapper->SetPointCloudMapper(mpLidarMapping);
    mpLoopCloser->SetPointCloudMapper(mpLidarMapping);
    mpTracker->SetPointCloudMapper(mpLidarMapping);
    mpLidarMapping->SetTracker(mpTracker);
  }
...
vector<float> System::GetTrackTimes() {
  std::unique_lock<std::mutex> lck(mMutexTrackTimes);
  return vTimesTrack;
}
void System::CreateFrameAndPush(
    const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp,
    const std::string &filename, const std::vector<IMU::Point> &ImuMeas,const std::vector<Eigen::Vector3f> &OdomMeas,
  ...
  auto frame =
      mpTracker->CreateFrame(imLeftToFeed, imRightToFeed, timestamp, filename);

  auto frame_wrapper =
      std::make_shared<FrameWrapper>(frame, ImuMeas, OdomMeas, PosePromise);
  {
    std::lock_guard<std::mutex> lck(mQueueMutex);
    mFrameQueue[timestamp] = frame_wrapper;
  }
  mQueueCond.notify_one();
}
void System::NotifyProcessingComplete() {
  mQueueCond.notify_all();  // 通知所有等待的线程
}
void System::CreateTrackFrameThread() {
  auto TrackFrame = [this]() {
    int fps = 0;
    float sum = 0.0;
    auto last_p = std::chrono::high_resolution_clock::now();
    while (!mbfinished) {
      std::unique_lock<std::mutex> lck(mQueueMutex);
      mQueueCond.wait(lck, [&]() {
        return (!mFrameQueue.empty() &&
                mFrameQueue.begin()->second != nullptr) ||
               mbfinished;
      });
      if (!mFrameQueue.empty() && mFrameQueue.begin()->second != nullptr) {
        std::shared_ptr<FrameWrapper> framewrapper;
        framewrapper = mFrameQueue.begin()->second;
        mFrameQueue.erase(mFrameQueue.begin());
        lck.unlock();
        mQueueNotFullCond.notify_one();
        ...
        auto dura = std::chrono::duration_cast<std::chrono::milliseconds>(
                        now_p - last_p)
                        .count();
        std::unique_lock<std::mutex> track_lock(mMutexTrackTimes);
        double track_time = dura / 1000.0f;  // ms -> s
        vTimesTrack.emplace_back(track_time);
        track_lock.unlock();
        if (dura >= 10) {
          last_p = now_p;
          // printf("fps: %f\n", fps * 1000.0f / dura);
        ...
        // 统计平均fps
        // printf("comsume time: %f\n", coms);
      }
    }
  };
  mTrackThread = std::make_shared<std::thread>(TrackFrame);
}
Sophus::SE3f System::TrackStereo(std::shared_ptr<FrameWrapper> framewrapper) {
  // Check mode change
  {
  ...
  Sophus::SE3f Tcw = mpTracker->GrabImageStereo(*framewrapper->mFrame);
...
std::future<Sophus::SE3f> System::TrackStereoAsync(
    const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp,
  ...
  mthreadPool->PostTask(std::bind(&System::CreateFrameAndPush, this, imLeft,
                                  imRight, timestamp, filename, vImuMeas,vOdomMeas,
                                  pose_promise));
...
std::future<Sophus::SE3f> System::TrackRGBDAsync(
  ...
  if (mSensor != RGBD && mSensor != IMU_RGBD) {
    std::cerr << "ERROR: you called TrackRGBD "
                 "but input sensor was not set to RGBD."
              << std::endl;
    exit(-1);
  }
  if (mthreadPool == nullptr) {
  ...
  auto interval_ms =
      std::chrono::duration_cast<std::chrono::milliseconds>(now - mLastSubmitTime).count();
  const int kMinIntervalMs = 66;  // ≈15FPS
  if (interval_ms < kMinIntervalMs) {
    std::this_thread::sleep_for(std::chrono::milliseconds(kMinIntervalMs - interval_ms));
  }
  mLastSubmitTime = std::chrono::steady_clock::now();
  // 阻塞直到队列不满（改为等待）
  {
    std::unique_lock<std::mutex> lck(mQueueMutex);
    mQueueNotFullCond.wait(lck, [&]() {
      return mFrameQueue.size() < kMaxFrameQueueSize || mbfinished;
    });
    mFrameQueue[timestamp] = nullptr;  // 占位
  }
  auto pose_promise = std::make_shared<std::promise<Sophus::SE3f>>();
  // std::cout<<"odom size: "<<vOdomMeas.size()<<std::endl;
  mthreadPool->PostTask(std::bind(&System::CreateFrameAndPush, this, imLeft,
                                  depthmap, timestamp, filename, vImuMeas,vOdomMeas,
                                  pose_promise));
  return pose_promise->get_future();
}
...
Sophus::SE3f System::TrackRGBD(shared_ptr<FrameWrapper> framewrapper) {
  ...
  if (mSensor == System::IMU_RGBD) {
    ...
    // std::cout<<framewrapper->mOdomMeas.size()<<std::endl;
    if (framewrapper->mOdomMeas.size() > 0) {
      // std::cout << "GrabOdomData" << std::endl;
      for (size_t i = 0; i < framewrapper->mOdomMeas.size(); i++) {
        mpTracker->GrabOdomData(framewrapper->mOdomMeas[i]);
      }
    }
  }
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap,
                               const double &timestamp,
                               const vector<IMU::Point> &vImuMeas,
                               const vector<Eigen::Vector3f> &vOdomMeas,
                               string filename) {
  if (mSensor != RGBD && mSensor != IMU_RGBD) {
  ...
  if (vOdomMeas.size() > 0) {
    for (size_t i = 0; i < vOdomMeas.size(); i++) {
      mpTracker->GrabOdomData(vOdomMeas[i]);
    }
  }
  Sophus::SE3f Tcw =
  ...
  unique_lock<mutex> lock2(mMutexState);
  mTrackingState = mpTracker->mState;
  // unique_lock<mutex> lock(mMutexTrackedCnt);
  // mTrackLostCnt = mpTracker->mTrackLostCnt;
...
std::future<Sophus::SE3f> System::TrackMonocularAsync(
    ...
    const std::vector<IMU::Point> &vImuMeas,const std::vector<Eigen::Vector3f> &vOdomMeas, const std::string &filename) {
  if (mSensor != MONOCULAR && mSensor != IMU_MONOCULAR) {
    std::cerr
        << "ERROR: you called TrackMonocularAsync "
           "but input sensor was not set to Monocular nor Monocular-Inertial."
        << std::endl;
    exit(-1);
  }
  if (mthreadPool == nullptr) {
    mthreadPool = std::make_shared<hobot::CThreadPool>();
    mthreadPool->CreateThread(2);
    cout << "CreateTrackFrameThread" << endl;
    CreateTrackFrameThread();
  }
  {
    std::lock_guard<std::mutex> lck(mQueueMutex);
  ...
  auto pose_promise = std::make_shared<std::promise<Sophus::SE3f>>();
  mthreadPool->PostTask(std::bind(&System::CreateFrameAndPush, this, im,
                                  cv::Mat(), timestamp, filename, vImuMeas,vOdomMeas,
                                  pose_promise));
  return pose_promise->get_future();
}
...
void System::Shutdown(std::string save_dir) {
  {
    unique_lock<mutex> lock(mMutexReset);
    mbShutDown = true;
  }
  ...
  // mStrSaveAtlasToFile = save_dir + "/" + mStrSaveAtlasToFile;
  ...
  // if (mpViewer) pangolin::BindToContext("Horizon-SLAM: Map Viewer");

#ifdef REGISTER_TIMES
  mpTracker->PrintTimeStats(save_dir);
#endif
}
...
void System::SaveTrajectory(const string &filename) {
  ...
  for (list<Sophus::SE3f>::iterator
    ...
    Eigen::Vector3f twc = Twc.translation();  // cameara to world
...
void System::SaveFrame2FrameReprojErr(const string &filename) {
  // 保存mpTracker->mFrame2FrameReprojErr和mpTracker->mFrame2MapReprojErr
  cout << endl
       << "Saving frame2frame reproj error to " << filename << " ..." << endl;
  ofstream f;
  f.open(filename.c_str());
  f << fixed;
  for (auto &frame : mpTracker->mFrame2FrameReprojErr) {
    int id = frame.first;
    vector<float> err = frame.second;
    f << id << " ";
    for (auto e : err) {
      f << e << " ";
    }
...
void System::SaveFrame2MapReprojErr(const string &filename) {
  // 保存mpTracker->mFrame2FrameReprojErr和mpTracker->mFrame2MapReprojErr
  cout << endl
       << "Saving frame2map reproj error to " << filename << " ..." << endl;
  ofstream f;
  f.open(filename.c_str());
  f << fixed;
  for (auto &frame : mpTracker->mFrame2MapReprojErr) {
    int id = frame.first;
    vector<float> err = frame.second;
    f << id << " ";
    for (auto e : err) {
...
void System::SaveTrajectoryTUM(const string &filename) {
  ...
  // std::cout << " The Size of KeyFrame is : " << vpKFs.size() << std::endl;
  // Transform all keyframes so that the first keyframe is at the origin.
  // After a loop closure the first keyframe might not be at the origin.
  Sophus::SE3f Two = vpKFs[0]->GetPoseInverse();  // world to keyframe
  ...
  for (list<Sophus::SE3f>::iterator
    ...
    f << setprecision(4) << (*lT) * 1e3 << " " << setprecision(9) << twc(0)
      << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " "
      << q.z() << " " << q.w() << endl;
  }
  f.close();
...
void System::SaveKeyFrameTrajectoryTUM(const string &filename) {
  ...
  f.open(filename.c_str());
  f << fixed;
  cout << "The size of keyframe is : " << vpKFs.size() << endl;
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    f << setprecision(4) << pKF->mTimeStamp * 1e3 << setprecision(9) << " "
      << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y()
      << " " << q.z() << " " << q.w() << endl;
  }
...
void System::SaveTrackTimeCost(const string &filename,
                               const vector<float> &timecost) {
  cout << "Saving track time cost to " << filename << " ..." << endl;
  ofstream f;
  f.open(filename.c_str());
  f << fixed;
  for (size_t i = 0; i < timecost.size(); i++) {
    f << setprecision(4) << timecost[i] << endl;
  }
  f.close();
  cout << "track time cost saved!" << endl;
}
void System::SaveTrajectoryTUM2(const string &filename) {
  cout << endl << "Saving camera trajectory to " << filename << " ..." << endl;
  // if(mSensor==MONOCULAR)
  // {
  //     cerr << "ERROR: SaveTrajectoryTUM cannot be used for monocular." <<
  ...
  for (list<Sophus::SE3f>::iterator
    ...
    f << setprecision(4) << (*lT) << " " << setprecision(9) << twc(0) << " "
      << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z()
      << " " << q.w() << endl;
  }
  f.close();
...
void System::SaveKeyFrameTrajectoryTUM2(const string &filename) {
  cout << endl
       << "Saving keyframe trajectory to " << filename << " ..." << endl;
  ...
  for (size_t i = 0; i < vpKFs.size(); i++) {
    ...
    Eigen::Quaternionf q = Twc.unit_quaternion();
    Eigen::Vector3f t = Twc.translation();
    f << setprecision(4) << pKF->mTimeStamp << setprecision(9) << " " << t(0)
      << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y() << " "
      << q.z() << " " << q.w() << endl;
  }
...
LoopClosing *System::GetLoopCloserVal() { return mpLoopCloser; }
Settings *System::GetSetting() { return settings_; }

void System::SaveKeyFrameLoopTrajectoryTUM(const string &filename) {
  cout << endl
       << "Saving keyframe trajectory to " << filename << " ..." << endl;

  set<KeyFrame *> vpKFs = GetLoopCloserVal()->mpMatchedKF->mspLoopEdges;
  // sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
  ...
  for (auto pKF = vpKFs.begin(); pKF != vpKFs.end(); ++pKF) {
    // KeyFrame* pKF = vpKFs.;
    ...
    Eigen::Quaternionf q = Twc.unit_quaternion();
    Eigen::Vector3f t = Twc.translation();
    f << setprecision(6) << (*pKF)->mTimeStamp * 1e3 << setprecision(9) << " "
      << t(0) << " " << t(1) << " " << t(2) << " " << q.x() << " " << q.y()
      << " " << q.z() << " " << q.w() << endl;
  }
...
void System::SaveKeyFrameLoopTrajectoryTUM2(const string &filename) {
  cout << endl
       << "Saving keyframe trajectory to " << filename << " ..." << endl;
...
int System::GetTrackLostCnt() {
  unique_lock<mutex> lock(mMutexTrackedCnt);
  return mTrackLostCnt;
}
...
FrameDrawer *System::GetmpFrameDrawe() { return mpFrameDrawer; }
int System::GetFrameQueueSize() {
  std::lock_guard<std::mutex> lck(mQueueMutex);
...
void System::SaveAtlas(int type) {
  if (!mStrSaveAtlasToFile.empty()) {
    ...
    std::cout << "pathSaveFileName: " << pathSaveFileName << std::endl;
...
// void System::SaveForGridMap(const string &filename_traj, const string &filename_points)
// {
//     // 安全检查
//     if(!mpAtlas || !mpAtlas->GetCurrentMap())
//     {
//         cout << "ERROR: Map is not ready yet!" << endl;
//         return;
//     }
...
//     cout << endl << "Saving GridMap Data to: " << filename_traj << " ..." << endl;

//     // 1. 保存关键帧轨迹
//     ofstream f_traj;
//     f_traj.open(filename_traj.c_str());
//     f_traj << fixed;
...
//         // [Fix 1] 类型匹配修正: KeyFrame::GetCameraCenter 返回 Eigen::Vector3f
//         Eigen::Vector3f twc = pKF->GetCameraCenter();

//         // [Fix 2] 旋转矩阵处理: GetRotation 返回 Eigen::Matrix3f (Rcw)
//         // 我们需要 Rwc (相机->世界)，它是 Rcw 的转置
//         Eigen::Matrix3f Rcw = pKF->GetRotation();
//         Eigen::Matrix3f Rwc = Rcw.transpose();

//         // [Fix 3] 类型转换: Converter::toQuaternion 需要 cv::Mat
//         cv::Mat Rwc_cv = Converter::toCvMat(Rwc);
//         vector<float> q = Converter::toQuaternion(Rwc_cv);

//         // 使用 Eigen 的 () 访问符，而不是 cv::Mat 的 .at()
//         f_traj << setprecision(6) << pKF->mTimeStamp << " "
...
//     // 2. 保存地图点和观测关系
//     ofstream f_pts;
//     f_pts.open(filename_points.c_str());
//     f_pts << fixed;
...
//         // [Fix 4] 类型匹配: GetWorldPos 返回 Eigen::Vector3f
//         Eigen::Vector3f pos = pMP->GetWorldPos();
        
//         f_pts << setprecision(9) << pos(0) << " " << pos(1) << " " << pos(2);

//         // [Fix 5] 迭代器类型修正: ORB-SLAM3 中 value 是 tuple<int, int>
//         // 查看 MapPoint.h: std::map<KeyFrame*, std::tuple<int, int>> GetObservations();
//         std::map<KeyFrame*, std::tuple<int, int>> observations = pMP->GetObservations();
        
//         // 使用 auto 自动推导迭代器类型，避免编译报错
//         for(auto it = observations.begin(); it != observations.end(); ++it)
//         {
//             KeyFrame* pKF = it->first;
//             if(pKF && !pKF->isBad())
//             {
//                 f_pts << " " << setprecision(6) << pKF->mTimeStamp;
//             }
//         }
//         f_pts << endl;
//     }
//     f_pts.close();
//     cout << "GridMap Data Saved Successfully!" << endl;
// }
