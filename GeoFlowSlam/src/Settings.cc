...
void Settings::readCamera1(cv::FileStorage& fSettings) {
  ...
  std::string cameraModel =
      readParameter<string>(fSettings, "Camera.type", found);
  mCameraModel_ = readParameter<string>(fSettings, "Camera.type", found);
  enableOpticalFlow_ = readParameter<int>(fSettings, "UseOpticalFlow", found);
  enableClahe_ = readParameter<int>(fSettings, "UseClahe", found);
  kfInsertInterval_ =
      readParameter<float>(fSettings, "KFInsertInterval", found);
  lkWinsize_ = readParameter<int>(fSettings, "LKWindowSize", found, false);
  F_THRESHOLD_ = readParameter<float>(fSettings, "F_THRESHOLD", found, false);
  MASK_THRESHOLD_ =
      readParameter<int>(fSettings, "MASK_THRESHOLD", found, false);
  enableGMSInit_ = readParameter<int>(fSettings, "UseGMSInit", found, false);
  enableICP_ = readParameter<int>(fSettings, "UseICP", found, false);
  imageDownsample_ = readParameter<int>(fSettings, "Downsample", found, false);
  enableICPLoop_ = readParameter<int>(fSettings, "UseICPLoop", found, false);
  enableICPLocalBA_ =
      readParameter<int>(fSettings, "UseICPLocalBA", found, false);
  enableLidarLocalBA_ =
      readParameter<int>(fSettings, "UseLidarLocalBA", found, false);
  downsizeResolution_ =
      readParameter<float>(fSettings, "VoxelResolution", found, false);
  useLidarObs_ =
      readParameter<int>(fSettings, "UsePointCloudObs", found, false);
  iterMax_ = readParameter<int>(fSettings, "MaxIterations", found, false);
  enableViewer_ = readParameter<int>(fSettings, "UseViewer", found, false);
...
void Settings::readIMU(cv::FileStorage& fSettings) {
  ...
  imuInitMethod_ = readParameter<int>(fSettings, "IMU.IMUMethod", found, false);
  fast_init_ = readParameter<int>(fSettings, "IMU.FasterInit", found, false);
  imuInitMethod_ = (imuInitMethod_ > 2 ? 0 : imuInitMethod_);
  cv::Mat cvTbc = readParameter<cv::Mat>(fSettings, "IMU.T_b_c1", found);
  Tbc_ = Converter::toSophus(cvTbc);
  timeRecentlyLost_ =
      readParameter<float>(fSettings, "IMU.TimeRecentlyLost", found);
...
void Settings::readRGBD(cv::FileStorage& fSettings) {
  ...
  bf_ = b_ * calibration1_->getParameter(0);
  timeRecentlyLost_ =
      readParameter<float>(fSettings, "TimeRecentlyLost", found);
  downsizeResolution_ =
      readParameter<float>(fSettings, "VoxelResolution", found, false);
  globalResolution_ = readParameter<float>(
      fSettings, "LidarMapping.GlobalResolution", found, false);
  localResolution_ = readParameter<float>(
      fSettings, "LidarMapping.LocalResolution", found, false);
  lidarConfigFile_ =
      readParameter<string>(fSettings, "LidarMapping.ConfigFile", found, false);
  icpMethod_ = readParameter<string>(fSettings, "ICPMethod", found, false);
  enableRobotOdom_ =
      readParameter<int>(fSettings, "UseRobotOdom", found, false);
  insertKFsWhenLost_ =
      readParameter<int>(fSettings, "InsertKFsWhenLost", found, false);
  cv::Mat R_odom2cam =
      readParameter<cv::Mat>(fSettings, "T_odom_w", found, false);
  eigenRodom2cam_ = Converter::toMatrix3f(R_odom2cam);
}
...
void Settings::readOtherParameters(cv::FileStorage& fSettings) {
  ...
  extractor_tpye_ =
      readParameter<std::string>(fSettings, "ORBextractor.type", found, false);
}
...
ostream& operator<<(std::ostream& output, const Settings& settings) {
  ...
  if (settings.sensor_ == System::STEREO ||
    ...
    // output << "\t-whether use optical flow " << settings.enableOpticalFlow_
    //        << endl;
  ...
  if (settings.sensor_ == System::IMU_MONOCULAR ||
    ...
    output << "\t-IMU Initializer: "
           << (settings.imuInitMethod_ == 0
                   ? "orb_slam"
                   : (settings.imuInitMethod_ == 1 ? "vig_init"
                                                   : "imu_initializer"))
           << endl;
  }
  ...
  if (settings.sensor_ == System::RGBD ||
    ...
    output << "\t-Resolution for voxel " << settings.downsizeResolution_
           << endl;
    output << "\t-LidarMapping global resolution: "
           << settings.globalResolution_ << endl;
    output << "\t-LidarMapping local resolution: " << settings.localResolution_
           << endl;
  }
  ...
  output << "\t-KeyFrame insert interval " << settings.kfInsertInterval_
         << endl;
  output << "\t-LK window size " << settings.lkWinsize_ << endl;
  output << "\t-Whether use optical flow " << settings.enableOpticalFlow_
         << endl;
  output << "\t-Whether use CLAHE " << settings.enableClahe_ << endl;
  output << "\t-Threshold for F matrix " << settings.F_THRESHOLD_ << endl;
  output << "\t-Threshold for mask " << settings.MASK_THRESHOLD_ << endl;
  output << "\t-Whether use GMS init " << settings.enableGMSInit_ << endl;
  output << "\t-Whether use ICP " << settings.enableICP_ << endl;
  output << "\t-Whether use ICP loop " << settings.enableICPLoop_ << endl;
  output << "\t-Whether use ICP local BA " << settings.enableICPLocalBA_
         << endl;
  output << "\t-Whether use point cloud observation " << settings.useLidarObs_
         << endl;
  return output;
}
};  // namespace ORB_SLAM3
