...
LoopClosing::LoopClosing(Atlas* pAtlas, KeyFrameDatabase* pDB,
                         ORBVocabulary* pVoc, Settings* pSettings,
...
void LoopClosing::Run() {
  mbFinished = false;
  mbUseICPConstraint = mpSettings->enableICPLoop();
  while (1) {
    ...
    if (CheckNewKeyFrames()) {
      ...
      if (bFindedRegion) {
        ...
        if (mbLoopDetected) {
          ...
          float dt = abs(mpLoopMatchedKF->mTimeStamp - mpCurrentKF->mTimeStamp);
          ...
          Eigen::Vector3d dp = g2oSww_new.translation();
          // 距离过大，不接受
          // if (dt < 30.0 || phi.norm() > 0.349f)
          // bGoodLoop = false;
          if (mpCurrentKF->GetMap()->IsInertial()) {
            // Sophus::SE3d Twc = mpCurrentKF->GetPoseInverse().cast<double>();
            // g2o::Sim3 g2oTwc(Twc.unit_quaternion(), Twc.translation(), 1.0);
            ...
            // fabs(phi(2))<0.349f)
            if (fabs(phi(0)) < 0.1f && fabs(phi(1)) < 0.1f &&
                fabs(phi(2)) < 1.0f) {
              if (mpCurrentKF->GetMap()->IsInertial()) {
                // If inertial, force only yaw
                if ((mpTracker->mSensor == System::IMU_MONOCULAR ||
...
bool LoopClosing::DetectCommonRegionsFromBoW(
  ...
  for (KeyFrame* pKFi : vpBowCand) {
    ...
    if (numBoWMatches >= nBoWMatches)  // TODO pick a good threshold
    {
        ...
        if (numProjMatches >= nProjMatches) {
          ...
          if (numOptMatches >= nSim3Inliers) {
            g2o::Sim3 gSmw(pMostBoWMatchesKF->GetRotation().cast<double>(),
                           ...
                           1.0);
            g2o::Sim3 gScw = gScm * gSmw;  // Similarity matrix of current from
                                           // the world position
            Sophus::Sim3f mScw = Converter::toSophus(gScw);
      ...
      /*else
      {
          Verbose::PrintMess("BoW candidate: it don't match with the
      current one", Verbose::VERBOSITY_DEBUG);
      }*/
    }
    index++;
  }
...
void LoopClosing::CorrectLoop() {
  ...
  while (!mpLocalMapper->isStopped()) {
    // cout << "Wait until Local Mapping has effectively stopped" << endl;
    usleep(1000);
  }
  ...
  std::cout << "Loop: number of connected KFs -> " +
                   to_string(mvpCurrentConnectedKFs.size())
            << std::endl;
  // 数量过多会导致mpLocalMapper一直被占用，导致无法插入新的关键帧
  // if (mvpCurrentConnectedKFs.size() > 50) {
...

  {
    ...
    cout << "Correcting keyframes" << endl;
    ...
    cout << "Correcting MapPoints" << endl;
    ...
    cout << "Correcting MapPoints done" << endl;
    // Start Loop Fusion
  ...
  cout << "Update connections done" << endl;
  cout << "Loop connections: " << LoopConnections.size() << endl;
  // Optimize graph
  bool bFixedScale = mbFixScale;
  ...
  if (pLoopMap->IsInertial() && pLoopMap->isImuInitialized()) {
    Optimizer::OptimizeEssentialGraph4DoF(
        pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3,
        LoopConnections, mbUseICPConstraint);
  } else {
    cout << "Loop -> Scale correction: " << mg2oLoopScw.scale() << endl;
    Optimizer::OptimizeEssentialGraph(
        pLoopMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3,
        LoopConnections, bFixedScale, mbUseICPConstraint);
  }
#ifdef REGISTER_TIMES
  ...
  if (mpTracker->mSensor == System::RGBD ||
      mpTracker->mSensor == System::IMU_RGBD) {
    mpLidarMapping->mabIsUpdating = false;  // 强制让已有的更新停止，进行新的
    mpThreadDML = new thread(&LidarMapping::updatecloud, mpLidarMapping,
                             std::ref(*mpCurrentKF->GetMap()));
    mpThreadDML->detach();
...
void LoopClosing::MergeLocal() {
  ...
  // std::cout << "[Merge]: Ma = " << to_string(pCurrentMap->GetId()) << ";
  // #KFs = " << to_string(spLocalWindowKFs.size()) << "; #MPs = " <<
  ...
  if (vpCurrentMapKFs.size() == 0) {
    ...
    if (mpTracker->mSensor != System::MONOCULAR) {
      Optimizer::OptimizeEssentialGraph(
          mpCurrentKF, vpMergeConnectedKFs, vpLocalCurrentWindowKFs,
          vpCurrentMapKFs, vpCurrentMapMPs, mbUseICPConstraint);
    }
...
void LoopClosing::MergeLocal2() {
  ...
  << endl; cout << "spMapPointMerge.size() " << spMapPointMerge.size() <<
  endl;*/
  ...
  // tree
  {
    ...
    if (!mbStopGBA) {
      ...
      while (!lpKFtoCheck.empty()) {
        ...
        // cout << "---Updating KF " << pKF->mnId << " with " <<
        // sChilds.size() << " childs" << endl; cout << " KF
        // mnBAGlobalForKF: " << pKF->mnBAGlobalForKF << endl;
        ...
        for (set<KeyFrame*>::const_iterator sit = sChilds.begin();
          ...
          if (pChild->mnBAGlobalForKF != nLoopKF) {
            // cout << "++++New child with flag " <<
            // pChild->mnBAGlobalForKF << "; LoopKF: " << nLoopKF <<
            ...
            // cout << "Child bias: " << pChild->GetImuBias() <<
            // endl;
        ...
        " meters" << endl; double desvX = 0; double desvY = 0; double
        desvZ = 0; if(pKF->mbHasHessian)
        {
        ...
        pKF->GetConnectedKeyFrames().size() << " connected KFs" << endl;
            cout << "--It has " << pKF->GetCovisiblesByWeight(80).size()
        << " connected KF with 80 common matches or more" << endl; cout
        << "--It has " << pKF->GetCovisiblesByWeight(50).size() << "
        connected KF with 50 common matches or more" << endl; cout <<
        "--It has " << pKF->GetCovisiblesByWeight(20).size() << "
        connected KF with 20 common matches or more" << endl;
        ...
        desvY << ", " << desvZ << endl;
            ...
            cv::Mat imLeft = cv::imread(strNameFile,
        CV_LOAD_IMAGE_UNCHANGED);
            ...
            for(int i=0; i<vpMapPointsKF.size(); ++i)
            {
                ...
                num_MPs += 1;
                string strNumOBs =
        to_string(vpMapPointsKF[i]->Observations()); cv::circle(imLeft,
        pKF->mvKeys[i].pt, 2, cv::Scalar(0, 255, 0));
        ...
        "_KF" + to_string(pKF->mnId) +"_D" + to_string(dist) +".png";
      ...
      if (mpTracker->mSensor == System::RGBD ||
          mpTracker->mSensor == System::IMU_RGBD) {
        mpLidarMapping->mabIsUpdating =
            false;  // 强制让已有的更新停止，进行新的
        mpThreadDML = new thread(&LidarMapping::updatecloud, mpLidarMapping,
                                 std::ref(*pActiveMap));
        mpThreadDML->detach();
        cout << "Map updated!" << endl;
